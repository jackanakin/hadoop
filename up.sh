...
docker run -it -d --name hadoop-slave-1 --network=hadoop_network jackanakin:hadoop-slave
docker run -it -d --name hadoop-slave-2 --network=hadoop_network jackanakin:hadoop-slave
docker run -it -d --name hadoop-master --network=hadoop_network jackanakin:hadoop-master
